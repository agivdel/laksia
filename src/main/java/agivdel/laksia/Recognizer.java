...
public class Recognizer {
    public static Map<Integer, String> trainModel(String pathName) {
        ...
        if (!trainingSetDirectory.exists()) {
            System.out.println("проверьте наличие директории " +
                    "src/main/resources/trainingSet_facerec/" +
                    " с обучающим набором изображений .png");
        }
        ...
        if (imageFiles.length <= 1) {
            System.out.println("слишком мало изображений для тренировки модели распознавания лиц");
            return new HashMap<>();
        }
    ...
    public static double[] faceRecognize(Mat currentFace) {
        ...
        if (!Files.exists(LBPHTrainedModelPath, LinkOption.NOFOLLOW_LINKS)) {
            System.out.println("отсутствует модель распознавания лиц\nраспознавание лиц невозможно");
